...
The `Gateway` interface in `gateway.go` covers all methods including `BatteryInventory` and `EnableHighFrequencyMode`. Embed it in test doubles or mock implementations.
...
`stream.go` contains a single exported function:

- `EnableHighFrequencyMode`: POST `{"enable":1}` to `/ivp/livedata/stream`, verify `sc_stream == "enabled"`. This switches the gateway's internal data collection cycle from ~5 seconds to ~1 second. The mode persists across calls but resets on reboot. After calling this, use `LiveData` in a polling loop — there is no persistent connection or push mechanism.
...
| `stream.go` | `POST /ivp/livedata/stream` | Enables ~1-second gateway data refresh; poll `LiveData` in a loop after calling |
